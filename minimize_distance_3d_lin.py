...
def solve_model(N, dt, x_min, x_max, y_min, y_max, z_min, z_max, v_max, a_max, obstacles, zones, drone_w, x_start, y_start, z_start, x_goal, y_goal, z_goal):
    ...
    model.s_z = pyo.Var(model.steps, domain=pyo.PositiveReals)
    ...
    def SSMx(model, i):
        return model.x[0,i+1] == model.x[0,i] + dt * model.x[3,i] + 0.5 * dt ** 2 * model.u[0,i]
    @model.Constraint(range(0,N))
    def SSMy(model, i):
        return model.x[1,i+1] == model.x[1,i] + dt * model.x[4,i] + 0.5 * dt ** 2 * model.u[1,i]
    @model.Constraint(range(0,N))
    def SSMz(model, i):
        return model.x[2,i+1] == model.x[2,i] + dt * model.x[5,i] + 0.5 * dt ** 2 * model.u[2,i]
    @model.Constraint(range(0,N))
    def SSMvx(model, i):
        return model.x[3,i+1] == model.x[3,i] + dt * model.u[0,i]
    @model.Constraint(range(0,N))
    def SSMvy(model, i):
        return model.x[4,i+1] == model.x[4,i] + dt * model.u[1,i]
    @model.Constraint(range(0,N))
    def SSMvz(model, i):
        return model.x[5,i+1] == model.x[5,i] + dt * model.u[2,i]
    ...
    model.s_z_pos_con = pyo.Constraint(model.steps, rule=lambda model, i: model.s_z[i] >= model.x[2,i]-z_goal)
    model.s_z_neg_con = pyo.Constraint(model.steps, rule=lambda model, i: model.s_z[i] >= -model.x[2,i]+z_goal)
    ...
    alpha = 1000
    obj_expr = sum(model.s_x[i] + model.s_y[i] + model.s_z[i] for i in model.steps) + sum(model.zone[i,j]*alpha for i in range(len(zones)) for j in model.steps)
    ...
    if results.solver.status == pyo.SolverStatus.ok and results.solver.termination_condition == pyo.TerminationCondition.optimal:
        ...
        return trajectory_x, trajectory_y, trajectory_z, dt
    else:
        return -1, -1, -1, -1
...
def main():
    ...
    # Solving the model
    x, y, z, dt = solve_model(N, dt, x_min, x_max, y_min, y_max, z_min, z_max, v_max, a_max, obstacles, zones, drone_w, x_start, y_start, z_start, x_goal, y_goal, z_goal)
