...
def solve_model(N, dt, x_min, x_max, y_min, y_max, z_min, z_max, v_max, a_max, obstacles, zones, drone_w, x_start, y_start, z_start, x_goal, y_goal, z_goal):
    ...
    alpha = 1000
    obj_expr = sum(pyo.sqrt((model.x[0,i]-x_goal)**2 + (model.x[1,i]-y_goal)**2 + (model.x[2,i]-z_goal)**2) for i in model.steps) + sum(model.zone[i,j]*alpha for i in range(len(zones)) for j in model.steps)
