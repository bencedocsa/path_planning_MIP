...
def solve_model(N, x_min, x_max, y_min, y_max, z_min, z_max, v_max, a_max, obstacles, zones, drone_w, x_start, y_start, z_start, x_goal, y_goal, z_goal):
    ...
    def SSMx(model, i):
        return model.x[0,i+1] == model.x[0,i] + model.dt * model.x[3,i] + 0.5 * model.dt2 * model.u[0,i]
    @model.Constraint(range(0,N))
    def SSMy(model, i):
        return model.x[1,i+1] == model.x[1,i] + model.dt * model.x[4,i] + 0.5 * model.dt2 * model.u[1,i]
    @model.Constraint(range(0,N))
    def SSMz(model, i):
        return model.x[2,i+1] == model.x[2,i] + model.dt * model.x[5,i] + 0.5 * model.dt2 * model.u[2,i]
    @model.Constraint(range(0,N))
    def SSMvx(model, i):
